...
typedef int64_t house;
typedef int64_t num_houses;

treap::treap<house, bool> housesLeft;
num_houses numHouses, curPos[300000];
char directions[300000];

int main() {
    num_houses i;
    scanf("%" PRId64 " %s", &numHouses, directions);
    curPos[numHouses-1] = 0;
    for (i = numHouses-2; i >= 0; i--) {
        if (directions[i] == 'L') curPos[i] = 1+curPos[i+1];
        else curPos[i] = 0;
    }
    
    REP(i, numHouses) housesLeft.addElementByKey(i, false, false);
    REP(i, numHouses-1) {
        printf("%" PRId64 "\n", housesLeft.getNodeAtPos(std::max(static_cast<num_houses>(0), curPos[i])).root->key+1);
        housesLeft.removeElementByPos(std::max(static_cast<num_houses>(0), curPos[i]));
    }
    printf("%" PRId64 "\n", housesLeft.root->key+1);
